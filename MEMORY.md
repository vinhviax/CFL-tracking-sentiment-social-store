...
- **So dem cua ingest run duoc cache tren bang `ingest_runs`** (`comment_count`, `analyzed_count`, `translated_zh_cn_count`, `data_start_date`, `data_end_date`, `counts_updated_at` - migration `0018`). Luat lam moi nam o `worker/src/services/runCounters.ts`: chi dem lai khi run chua tung dem, hoac con job `queued`/`running`/`failed`, hoac co job cap nhat sau lan dem gan nhat. **Khong duoc quay lai kieu tinh bang subquery moi request** - do la thu da lam sap production 2026-09-03. `counts_updated_at = NULL` nghia la "chua dem", tu dong dem lai o request ke tiep, nen cach ep dem lai la set cot nay ve NULL.
- **Migration phai chay TRUOC deploy** khi code moi doc/ghi cot moi. Vi du that: `routes/runs.ts` cua commit `c9c892a` ghi vao cot cua migration `0018`; deploy truoc migration se lam `/api/runs` tra 500.
